...
const RenderDomainAvailabilityItem = (props) => {
  const { data, hideStatus, onClick } = props;

  const formattedExpiredAt =
    data.status === DomainStatus.taken &&
    (data.expiredAt
      ? new Date(parseInt(data.expiredAt) * 1000).toLocaleDateString()
      : false);
  ...
  return (
    <div onClick={onClick} className={`social-item ${data.status}`}>
      <div className="social-main">
        <div className="social">
          <div
            className="icon"
            style={{
              background: SocialPlatformMapping(data.platform).color,
              color: "#fff",
            }}
          >
            <SVG
          ...
          {formattedExpiredAt && (
            <div className="domain-status">
              <div className="domain-status-key text-gray">Expiry</div>
              <div className="domain-status-value">{formattedExpiredAt}</div>
            </div>
          )}
        </div>
        <div className={`actions active`}>
          {shouldDisplayRegister && (
            <Link
              ...
              href={`${SocialPlatformMapping(data.platform).registerlink}${
                data.name
              }`}
              prefetch={false}
